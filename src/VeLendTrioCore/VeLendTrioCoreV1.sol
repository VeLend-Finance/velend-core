...
abstract contract VeLendTrioCoreV1 is FraxlendPairAccessControl, FraxlendPairConstants, ERC20, ReentrancyGuard, IFraxConstants {
    ...
    // +--------------------------------------------------------+
    // |               REPAY FROM YIELD FUNCTION                |
    // +--------------------------------------------------------+

    /// @notice The ```RepayFromYield``` event is emitted whenever a debt position is repaid from yield
    ...
    event RepayFromYield(address indexed payer, address indexed borrower, uint256 amountToRepay, uint256 shares);
