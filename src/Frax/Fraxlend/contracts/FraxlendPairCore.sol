...
abstract contract FraxlendPairCore is FraxlendPairAccessControl, FraxlendPairConstants, ERC20, ReentrancyGuard {
    ...
    /// @notice veFXS contract address;
    address public lockContractAddress;

    uint256 WEEK = 7 * 86_400;
    ...
    function _isSolvent(address _borrower, uint256 _exchangeRate) internal view returns (bool) {
        ...
        if (lockerAddresses[_borrower]) {
            ...
            LockedBalance lockedBalance = Locker(lockerAddresses[_borrower]).locked();

            /// @notice if your lock is expired then you can withdraw tokens.
            /// So then all your tokens is taking into account in current ltv
            if (lockedBalance.end < block.timestamp) {
                yield += lockedBalance.amount;
            }
        }

        /// @added using yield as collateral
        uint256 _ltv = (((_borrowerAmount * _exchangeRate) / EXCHANGE_PRECISION) * LTV_PRECISION) /
            (_collateralAmount + yield);
        return _ltv <= maxLTV;
    }
    ...
    /// @notice The ```CreateLock``` event fires when a user deposits assets to the pair
    ...
    event CreateLock(address indexed caller, address indexed owner, uint256 amount, uint256 untilAt);

    function creteLock(uint256 _value, uint256 _unlockTime) external override {
        uint256 unlockTime = (_unlockTime / WEEK) * WEEK;

        ILocker memory locker = new Locker(msg.sender, swapContractAddress, lockContractAddress);
        locker.createLock(_value, unlockTime);
    ...
    function repayFromYield(address _borrower) external {
        address lockerAddress = lockerAddresses[_borrower];

        // TODO: Сделать правильную проверку
        require(lockerAddress != 0x0, "Borrow didn't locked token through VeLend");

        Locker(lockerAddress).repayFromYield(_borrower);
    }
    ...
    function liquidate(
        ...
        // TODO: Не надо ли переводить
        // TODO: Учитывать тело лока, если block.timestamp > until;
        /// @added calculating yield from veFXS
        uint256 earnedYield = 0;
        address lockerAddress = lockerAddresses[_borrower];

        if (lockerAddress) {
            earnedYield = Locker(lockerAddress).earned();
        }
        ...
        uint256 _feesAmount;
        {
            ...
            /// @added veFXS yield is taking into account
            _leftoverCollateral = _leftoverCollateral + earnedYield;
